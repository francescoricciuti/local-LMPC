function calculateObstacleXY!(obstacle, trackCoeff, xy_track::Array{Float64,2}, i::Int64, j::Int64,orientation )
	...
	for l = 1:obstacle.n_obstacle
		...
		s_under = floor(Int64,obstacle.s_obstacle[i,j,l]/trackCoeff.ds)*trackCoeff.ds
		...
		if  s_over_ind > size(xy_track)[2]+1
			s_over_ind%=size(xy_track)[2]
			s_under_ind  = s_over_ind-1
		elseif s_over_ind == size(xy_track)[2]+1
			s_over_ind=1

		end
		weight1 = (obstacle.s_obstacle[i,j,l]-s_under)/trackCoeff.ds
		weight2 = (s_over-obstacle.s_obstacle[i,j,l])/trackCoeff.ds
		...
		obstacle.xy_vector[i,:,j,l] = xy_coord_infront *weight1+xy_coord_before*weight2
		...
		vector_sy = vec(normal_vec*obstacle.sy_obstacle[i,j,l])# explicit conversion to vector is necesary because the dimensions in next equations would not coincide Array{FLoat64,1}vs Array{FLoat64,2}
		obstacle.xy_vector[i,:,j,l] = obstacle.xy_vector[i,:,j,l] + vector_sy

		#points to plot the ry semi axis of the ellipsis
		vector_ry = vec(normal_vec*obstacle.ry)
		obstacle.axis_y_up[i,:,j,l] = obstacle.xy_vector[i,:,j,l] + vector_ry
		obstacle.axis_y_down[i,:,j,l] = obstacle.xy_vector[i,:,j,l] - vector_ry
