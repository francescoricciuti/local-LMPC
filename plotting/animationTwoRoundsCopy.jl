...
file2 ="data/2017-02-10-16-02-Data.jld"
copied_plot = 1
j=1
jj =1
...
Data2 = load(file2)
oldTraj2     = Data2["oldTraj"]
obstacle2 = Data2["obstacle"]
...
xy_pred2 = zeros(mpcParams.N+1,2,size(t)[1],oldTraj.n_oldTraj)
obstOrientation2 = zeros(size(obstacle.xy_vector)[1],size(obstacle.xy_vector)[3],size(obstacle.xy_vector)[4])
for k = 1:oldTraj.n_oldTraj
    for i = 1:oldTraj.oldNIter[k]
        # caluclate the predicted XY postion of the car from the s-ey values
        xy_pred2[:,:,i,k] = calculatePredictedXY(oldTraj2.z_pred_sol[:,:,:,k], mpcParams, trackCoeff, xy_track, convert(Int64,i),k)
        #calculate the obstacle postion from the s-ey values
        obstOrientation2 = calculateObstacleXY!(obstacle2, trackCoeff, xy_track,i,k,obstOrientation ) #this funciton computes values for row i
    end
end
...
ax1= subplot2grid([5,2],[0,0], rowspan =4)
...
# grid()
ax2= subplot2grid([5,2],[0,1], rowspan =4)
...
#                 "#ff91d1"]
    ...
    #     if oldTraj2.copyInfo[i,1,j]>0.0 # if lambda of copied traj is greater 0.1 -> if traj s used for solving.
    #         ax3[:scatter](oldTraj2.copyInfo[i,3,j],0, color = "#DCDCDC",edgecolor="black", lw =0.5)
    #     end
    # end
    for i=1:oldTraj2.oldNIter[j]-1
...
carParts2=drawCar(ax2,[oldTraj2.oldTrajXY[2,1,jj],oldTraj2.oldTrajXY[2,2,jj],oldTraj2.oldTrajXY[2,5,jj]*180/pi], ego_color)
for p in carParts2
                ax2[:add_patch](p)
end
...
function animate(frame)
    ...
    #     #update axis of x-y plot every time step
        ...
        ax2[:set_xlim](xy_pred2[6,1,k,jj]-3, xy_pred2[6,1,k,jj]+3)
        ax2[:set_ylim](xy_pred2[6,2,k,jj]-3, xy_pred2[6,2,k,jj]+3)

        car_plot[:set_data](oldTraj.oldTrajXY[1:k,1,j], oldTraj.oldTrajXY[1:k,2,j])

        car_plot2[:set_data](oldTraj2.oldTrajXY[1:k,1,jj], oldTraj2.oldTrajXY[1:k,2,jj])
        ...
        pos =[oldTraj2.oldTrajXY[k,1,jj],oldTraj2.oldTrajXY[k,2,jj], oldTraj2.oldTrajXY[k,5,jj]*180/pi]
        updateCarParts(ax2,carParts2,pos)

        pred_plot2[:set_data](xy_pred2[:,1,k,jj],xy_pred2[:,2,k,jj])
        # plot(oldTraj.oldTrajXY[k,1,j],oldTraj.oldTrajXY[k,2,j])
    #     if copied_plot ==1
    # #     #######################
    # #     #copied plot
    # #     #######################
            s_curr_copied[:set_data]([oldTraj2.oldTraj[k,1,j],oldTraj2.oldTraj[k,1,j]],[0,1])
    #     else
